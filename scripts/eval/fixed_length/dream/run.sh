...
if [[ "${TASK}" == "all" ]]; then
  for task in gsm8k humaneval math500 mbpp; do
    echo "[info] running dream fixed_length mode=${MODE} task=${task}"
    TASK="${task}" "${SELF}"
  done
  exit 0
fi
...
case "${MODE}" in
  void)
    ...
    OUTPUT_ROOT="${OUTPUT_ROOT:-evals_results/dream_void_fixed_length}"
    EXTRA_ARGS=("ban_tokens=void" "cut=true")
    ;;
  rainbow)
    MODEL="${MODEL:-${MODEL_PATH:-${RAINBOW_MODEL}}}"
    OUTPUT_ROOT="${OUTPUT_ROOT:-evals_results/dream_rainbow_fixed_length}"
    ;;
  instruct)
    MODEL="${MODEL:-${MODEL_PATH:-Dream-org/Dream-v0-Instruct-7B}}"
    OUTPUT_ROOT="${OUTPUT_ROOT:-evals_results/dream_instruct_fixed_length}"
    ;;
