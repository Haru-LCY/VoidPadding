...
Usage:
  ...
  scripts/merge/merge_llada_lora.sh

Optional environment variables:
  TOKENIZER_PATH   Tokenizer path. Defaults to repo tokenizer_llada.
...
: "${BASE_MODEL:?Set BASE_MODEL to a LLaDA base/instruct model path or Hugging Face model id.}"
...
TOKENIZER_PATH="${TOKENIZER_PATH:-${REPO_ROOT}/tokenizer_llada}"
TORCH_DTYPE="${TORCH_DTYPE:-bfloat16}"
DEVICE="${DEVICE:-auto}"
PYTHON="${PYTHON:-python}"

"${PYTHON}" scripts/merge/merge_llada_lora.py \
