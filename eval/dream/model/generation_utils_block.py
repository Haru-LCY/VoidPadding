...
class DreamGenerationMixin:
    ...
    def diffusion_generate(
        ...
        **kwargs,
        ...
        void_expand_block_length = kwargs.get("void_expand_block_length", None)
    ...
    def _sample(
        ...
        original_num_blocks = None
        if block_length is not None and int(block_length) > 0 and gen_length % int(block_length) == 0:
            original_num_blocks = gen_length // int(block_length)
        if void_expand_block_length is None:
            void_expand_block_length = block_length if block_length is not None else 32
        ...
        if (
            ...
            and original_num_blocks is not None
            and original_steps == int(original_num_blocks)
            and gen_length % int(block_length) == 0
        ):
            steps = gen_length // int(block_length)
        ...
        print(f'effective_nfe: {used_steps}')
        print(f'configured_nfe: {steps}')
        if cut:
            x = _finalize_cut_output(x, prompt_length, gen_length, cut_boundaries, eos_token_id)
