...
try:
    ...
    # lm-eval-harness>=0.4.8 moved dtype helpers to utils_hf.
...
def _nfe_stats_path(rank: int) -> Optional[str]:
...
def _append_nfe_stats(path: Optional[str], records: list[dict]) -> None:
...
class Dream(LM):
    def __init__(
        ...
        ban_tokens: Optional[str] = None,
        cut: Optional[bool] = False,
        void_expand: Optional[bool] = False,
        void_expand_max_length: Optional[int] = None,
        void_expand_block_length: Optional[int] = None,
        void_expand_mode: Optional[str] = "dual_tail",
        void_expand_window: Optional[int] = None,
        void_expand_tau_nonvoid: Optional[float] = None,
        void_expand_tau_gap: Optional[float] = 0.0,
        void_expand_debug: Optional[bool] = False,
        **kwargs,
    ) -> None:
        super().__init__()
        ...
        self._create_model_and_tokenizer(pretrained, dtype, trust_remote_code, tokenizer_pretrained)
        ...
        self.block_length = as_optional_int(block_length if block_length is not None else block_size)
        ...
        self.void_expand_block_length = as_optional_int(void_expand_block_length)
        ...
        self.void_expand_tau_gap = float(as_optional_float(void_expand_tau_gap) or 0.0)
    ...
    def _create_model_and_tokenizer(self, pretrained, dtype, trust_remote_code, tokenizer_pretrained=None):
        self.model = (
        ...
        tokenizer_source = tokenizer_pretrained or pretrained
    ...
    def _generate_batch(self, prompts: List[str]) -> List[str]:
        self._last_nfe_records = []
        if self.if_apply_chat_template:
            prompts = [
                self.apply_chat_template([{"role": "user", "content": prompt}])
                for prompt in prompts
            ]
        else:
            if self.add_bos_token:
                prompts = [self.tokenizer.bos_token + p for p in prompts]
        # tokenize with list output first so we can patch rare OOV(None) ids safely.
        tokenized = self.tokenizer(prompts, padding=False)
        prompt_id_lists = tokenized["input_ids"]

        replacement_count = 0
        if any(any(tok_id is None for tok_id in seq) for seq in prompt_id_lists):
            space_ids = self.tokenizer(" ", add_special_tokens=False).get("input_ids", [])
            if not space_ids or space_ids[0] is None:
                raise RuntimeError("Tokenizer produced OOV(None) ids, but no valid space token id was found for fallback.")
            replacement_id = int(space_ids[0])
            fixed_lists = []
            for seq in prompt_id_lists:
                fixed_seq = []
                for tok_id in seq:
                    if tok_id is None:
                        replacement_count += 1
                        fixed_seq.append(replacement_id)
                    else:
                        fixed_seq.append(int(tok_id))
                fixed_lists.append(fixed_seq)
            prompt_id_lists = fixed_lists
            eval_logger.warning(
                "Replaced %s OOV(None) token ids with space token id=%s to avoid tokenizer tensor conversion failure.",
                replacement_count,
                replacement_id,
            )
        ...
        if pad_token_id is None:
            raise RuntimeError("Tokenizer pad_token_id is required for batched generation.")
        max_len = max(len(seq) for seq in prompt_id_lists)
        prompt_ids = torch.full(
            (len(prompt_id_lists), max_len),
            int(pad_token_id),
            dtype=torch.long,
        )
        for i, seq in enumerate(prompt_id_lists):
            if len(seq) == 0:
                continue
            prompt_ids[i, -len(seq):] = torch.tensor(seq, dtype=torch.long)
        ...
        generation_ids = self.model.diffusion_generate(
            ...
            void_expand_block_length=self.void_expand_block_length,
        ...
        if self.void_expand and getattr(generation_ids, "metadata", None) is not None:
            metadata = generation_ids.metadata
            print('void_expand_selected_gen_length: ', metadata.get("void_expand_selected_gen_length"))
            print('void_expand_probe_steps: ', metadata.get("void_expand_probe_steps"))
            print('void_expand_stop_reason: ', metadata.get("void_expand_stop_reason"))
        print('=' * 20, end='\n\n')

        metadata = getattr(generation_ids, "metadata", None) or {}
        sample_nfes = metadata.get("sample_effective_nfe")
        effective_nfe = metadata.get("effective_nfe")
        if not isinstance(sample_nfes, list) or len(sample_nfes) != len(prompts):
            fallback_nfe = int(effective_nfe) if effective_nfe is not None else int(self.diffusion_steps)
            sample_nfes = [fallback_nfe] * len(prompts)

        self._last_nfe_records = []
        for row, (prompt_ids_row, sequence, sample_nfe) in enumerate(
            zip(prompt_ids, generation_ids.sequences, sample_nfes)
        ):
            generated_ids = sequence[prompt_ids_row.shape[0]:]
            generated_length = int((generated_ids != self.tokenizer.eos_token_id).sum().item())
            self._last_nfe_records.append(
                {
                    "model": "dream",
                    "sample_in_batch": int(row),
                    "nfe": int(sample_nfe),
                    "nfe_scope": "decoding_only",
                    "configured_nfe": int(self.diffusion_steps),
                    "effective_batch_nfe": int(effective_nfe) if effective_nfe is not None else int(sample_nfe),
                    "expansion_probe_nfe": int(metadata.get("void_expand_probe_steps", 0) or 0),
                    "generated_length": generated_length,
                    "max_new_tokens": int(self.max_new_tokens),
                    "block_length": int(self.block_length) if self.block_length is not None else None,
                    "cut": bool(self.cut),
                    "void_expand": bool(self.void_expand),
                    "void_expand_selected_gen_length": metadata.get("void_expand_selected_gen_length"),
                    "void_expand_stop_reason": metadata.get("void_expand_stop_reason"),
                }
            )
        return responses
    ...
    def generate_until(self, requests: List[Instance], disable_tqdm: bool = False):
        ...
        for batch_idx in range(0, len(requests), self.batch_size):
            ...
            nfe_records = []
            for offset, record in enumerate(getattr(self, "_last_nfe_records", [])):
                enriched = dict(record)
                enriched["sample_index"] = int(batch_idx + offset)
                nfe_records.append(enriched)
