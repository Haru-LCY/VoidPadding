...
_ATTENTION_TRACE_STATE = None


def start_attention_trace_step(
    x: torch.Tensor,
    *,
    prompt_length: int,
    step: int,
    ...
    global _ATTENTION_TRACE_STATE
    trace_enabled = bool(os.environ.get("LLADA_ATTENTION_TRACE_DIR"))
    ablate_eos_attention = os.environ.get("LLADA_ABLATE_EOS_ATTENTION", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "y",
    }
    if not trace_enabled and not ablate_eos_attention:
        _ATTENTION_TRACE_STATE = None
        return
    trace_every = max(1, int(trace_every))
    generation = x[:, int(prompt_length):]
    eos_mask = x.eq(int(eos_token_id))
    query_mask = x.eq(int(mask_id))
    has_committed_eos = bool(eos_mask[:, int(prompt_length):].any().item())
    should_trace = trace_enabled and has_committed_eos and (int(step) % trace_every == 0)
    _ATTENTION_TRACE_STATE = {
        "enabled": should_trace,
        "ablate_eos_attention": ablate_eos_attention and has_committed_eos,
        "prompt_length": int(prompt_length),
        "step": int(step),
        "block": int(block),
        "inner_step": int(inner_step),
        "x_generation": generation.detach().cpu().to(torch.int32),
        "mask_id": int(mask_id),
...
def finish_attention_trace_step():
    global _ATTENTION_TRACE_STATE
    state = _ATTENTION_TRACE_STATE
    _ATTENTION_TRACE_STATE = None
    if state is None:
        return None
    return {
        ...
        "x_generation": state["x_generation"],
        "p_eos_generation": state.get("p_eos_generation"),
        "layers": state["layers"],
        "traced_attention": bool(state["enabled"]),
    }


def add_attention_trace_step_field(key: str, value):
    if _ATTENTION_TRACE_STATE is not None:
        _ATTENTION_TRACE_STATE[key] = value
...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        if (
            _ATTENTION_TRACE_STATE is not None
            and _ATTENTION_TRACE_STATE.get("enabled")
            and layer_past is None
            and query_len == key_len
        ):
            prompt_length = int(_ATTENTION_TRACE_STATE["prompt_length"])
            eos_mask = _ATTENTION_TRACE_STATE["eos_mask"][:, :key_len].to(device=q.device)
            if prompt_length < query_len and bool(eos_mask.any().item()):
                dbg_k = k
                if q.size(1) != dbg_k.size(1):
                    assert q.size(1) % dbg_k.size(1) == 0
                    dbg_k = dbg_k.repeat_interleave(q.size(1) // dbg_k.size(1), dim=1, output_size=q.size(1))
                query_slice = q[:, :, prompt_length:, :].float()
                key_slice = dbg_k.float()
                scores = torch.matmul(query_slice, key_slice.transpose(-2, -1)) / math.sqrt(q.size(-1))
                if attention_bias is not None:
                    scores = scores + attention_bias[:, :, prompt_length:query_len, :key_len].float()
                attn_probs = torch.softmax(scores, dim=-1)
                eos_mass = (attn_probs * eos_mask[:, None, None, :].float()).sum(dim=-1)
                _ATTENTION_TRACE_STATE["layers"].append(
                    {
                        "layer": int(self.layer_id),
                        "pos_mass": eos_mass[0].mean(dim=0).detach().cpu().to(torch.float32),
                        "head_mass": eos_mass[0].mean(dim=1).detach().cpu().to(torch.float32),
                    }
                )
        ...
        if (
            _ATTENTION_TRACE_STATE is not None
            and _ATTENTION_TRACE_STATE.get("ablate_eos_attention")
            and layer_past is None
            and query_len == key_len
        ):
            ablate_k = k
            ablate_v = v
            if q.size(1) != ablate_k.size(1):
                assert q.size(1) % ablate_k.size(1) == 0
                ablate_k = ablate_k.repeat_interleave(q.size(1) // ablate_k.size(1), dim=1, output_size=q.size(1))
                ablate_v = ablate_v.repeat_interleave(q.size(1) // ablate_v.size(1), dim=1, output_size=q.size(1))
            scores = torch.matmul(q.float(), ablate_k.float().transpose(-2, -1)) / math.sqrt(q.size(-1))
            if attention_bias is not None:
                scores = scores + attention_bias.float()
            eos_mask = _ATTENTION_TRACE_STATE["eos_mask"][:, :key_len].to(device=q.device)
            query_mask = _ATTENTION_TRACE_STATE["query_mask"][:, :query_len].to(device=q.device)
            block_mask = query_mask[:, None, :, None] & eos_mask[:, None, None, :]
            scores = scores.masked_fill(block_mask, torch.finfo(scores.dtype).min)
            att = torch.matmul(torch.softmax(scores, dim=-1).to(dtype=ablate_v.dtype), ablate_v)
        else:
