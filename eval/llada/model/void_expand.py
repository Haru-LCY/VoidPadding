...
def get_void_expand_candidate_lengths(gen_length, block_length, max_length):
    if int(block_length) <= 0:
        raise ValueError("void_expand requires block_length > 0.")
    if int(gen_length) < 1:
...
def run_void_expand_probe(
    ...
    *,
    ...
    effective_window = int(window) if window is not None else max(8, int(block_length) // 4)
    lengths = get_void_expand_candidate_lengths(gen_length, block_length, max_length)
    candidates = []
    selected = lengths[-1]
    ...
    for length in lengths:
        ...
        probe_logits = model(probe_x).logits
    ...
    return {
        ...
        "void_expand_block_length": int(block_length),
